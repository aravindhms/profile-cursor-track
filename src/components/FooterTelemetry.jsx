...
export default function FooterTelemetry({ faceCoords }) {
  return (
    <footer className="fixed bottom-6 sm:bottom-8 right-6 sm:right-10 z-30 flex flex-col items-end gap-2.5 pointer-events-none select-none">
      
      {/* SRE Operational Status Pill */}
      <div className="pointer-events-auto flex items-center gap-3 px-4 py-2 rounded-full bg-white/[0.08] backdrop-blur-[20px] border border-white/20 text-xs font-mono text-white/90 shadow-glass transition-all hover:bg-white/[0.12] hover:border-white/30">
        <span className="flex items-center gap-2">
          <span className="relative flex h-2 w-2">
            <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
            <span className="relative inline-flex rounded-full h-2 w-2 bg-emerald-500"></span>
          </span>
          <span className="text-[11px] tracking-wider uppercase font-semibold text-white">99.99% Availability</span>
        </span>
